(function($){
	jQuery.fn.datePicker = function(param){
		$(this).addClass("DC_Wdate");
		$(this).click(function(){
			new WdatePicker(this,param);
		});
	}
	
	$.dateRangePicker = function(param){
		$("#"+param.maxDate).addClass("DC_Wdate");
		$("#"+param.minDate).addClass("DC_Wdate");
		
		$("#"+param.maxDate).click(function(){
			var param2 = new Object();
			param2.minDate = param.minDate;
			param2.dateFormat = param.dateFormat;
			param2.showTime = param.showTime;
			new WdatePicker(this,param2);
		});
		$("#"+param.minDate).click(function(){
			var param2 = new Object();
			param2.maxDate = param.maxDate;
			param2.dateFormat = param.dateFormat;
			param2.showTime = param.showTime;
			new WdatePicker(this,param2);
		});
	...
	function WdatePicker(el ,param){
		var dateFmt,showTime,skin,minDate,maxDate;
		dateFmt = param.dateFormat;
		showTime = param.showTime;
		skin = param.skin;
		minDate = param.minDate==null ? null : "$"+param.minDate;
		maxDate = param.maxDate==null ? null : "$"+param.maxDate;
		...
		if(this.top.document.dateDiv==undefined){
			...
			this.dd.yInput.onblur=function(){
				if(parseInt(this.value)!=$d.obj.year){
					$d.obj.redraw();}this.className='yminput';
				};
				...
				WdatePicker.prototype.isClose=function(){//关闭时间控件
				...
				WdatePicker.prototype.hahafffff=function(){//关闭时间控件
					alert('11111');
					//--------------------------------------------------------------------------------------------------------------------
				}
				WdatePicker.prototype.bc=function(str,fmt){
					...
					var v=str.split(/\W+/);
					//判断如果日期没有分隔符的情况下,日期做比较要先截取
					if(v.length==1 && str.length>=8){
						v[0] = str.substring(0,4);
						v[1] = str.substring(4,6);
						v[2] = str.substring(6,8);
					}
					...
					for(var i=0;i<f.length;i++){
						if(v[i]){
							if(f[i].toLowerCase()=='%y'){
								this.year=parseInt(v[i],10);if(isNaN(this.year)){
									this.year=this.aa;
								}
							}else if(f[i]=='%M'){
				...
				WdatePicker.prototype._fillmonth=function(){
					...
					for(i=0;i<6;i++){
						s.a("<tr><td ");
						ac=(ax)||(af&&t[i]>=this.minDate.month)||(ae&&t[i]<=this.maxDate.month);
						...
						s.a("<td ");
						ac=(ax)||(af&&t[i+6]>=this.minDate.month)||(ae&&t[i+6]<=this.maxDate.month);
				...
				WdatePicker.prototype.ad=function(s,len){
					...
					WdatePicker.prototype.aw=function(ab){
						...
						//判断如果日期没有分隔符的情况下,日期做比较要先截取
						if(s.length==1 && ab.length>=8){
							s[0] = ab.substring(0,4);
							s[1] = ab.substring(4,6);
							s[2] = ab.substring(6,8);
						}
					...
					WdatePicker.prototype.l=function(sDateTime){
						...
						Y=(Y==undefined)?'2000':Y;M=(M==undefined)?'1':M;D=(D==undefined)?'1':D;
					...
					WdatePicker.prototype.av=function(){
						var s=new __sb();
						s.a("<div id=dpTime style='"+((this.showTime)?'':'display:none;')+"float:left;margin-top:3px'><table cellspacing=0 cellpadding=0 border=0><tr><td rowspan=2>");
						...
						s.a("<input id=dpOkInput type=button style='height:20px;width:45%'></input><input id=dpEmptyInput value='清空' type=button style='height:20px;width:45%'></input>");
						s.a("</div>");
						return s.j();
					};
					...
					WdatePicker.prototype._setEmptyInput=function(){
						var d=$d.obj;
						$d.emptyInput.onclick=function(){
							$d.obj.emptyDate();
						};
					};
					WdatePicker.prototype.u=function(){
						...
						if((this.year*100+this.month*1)<(this.minDate.year*100+this.minDate.month*1)){
							this.dd.yInput.value=e=this.minDate.year;this.dd.mInput.value=tempMonth=this.minDate.month;
						...
						var ss=new Array();for(i=0;i<7;i++){
						...
						var bh=!aj&&!ai;for(i=1,j=bb;i<7;i++){
							s.a("<tr>");
							for(k=0;k<7;k++){
								if(j>=1&&j<=lastDate){
									if(bk&&(j==this.bs)){
									...
									s.a("<td align=center ");
									if(bh||((aj&&j>=this.minDate.date)||(ai&&j<=this.maxDate.date))){
										s.a("ondblclick=\"$d.obj.pickDate2(null,null,"+j+");\" ");
					...
					WdatePicker.prototype.pickDate2=function(Y,M,D,h,m,s){
						if(Y==null){
					...
					WdatePicker.prototype.emptyDate=function(){
						this.eCont.value="";
					};
						...
						function disposeDatePicker(){
							if($d!=undefined&&$d.obj!=undefined&&$d.obj.eCont!=window.event.srcElement&&$d.style.display!='none'){
								...
								if(x<rc.left||x>rc.right||y<rc.top||y>rc.bottom){
									...
									$d.yDiv.style.display=$d.mDiv.style.display='none';
									$d.ifr.style.display='none';$d.style.display='none';
								}else{
									$d.yDiv.style.display=$d.mDiv.style.display='none';
								}
							}
