 ...
 */
// ServletContext表示Servlet应用程序。每个Web应用程序只有一个context。在分布式环境中，一个应用程序同时部署到多个容器中，
// 并且每台java虚拟机都有一个ServletContext对象。在ServletConfig中调用getServletContext方法。有了ServletContext之后，就可
// 以共享能通过应用程序的所有资源访问的信息，促进Web对象的动态注册。前者是通过一个内部Map中的对象保存在ServletContext中来
// 实现的。
